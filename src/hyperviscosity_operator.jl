function hyperviscosity_operator(k_deriv, X, Y, p, n, polydeg)
    ...
    idxs_x = [convert.(Int, idxs_x[x]) for x in 1:length(idxs_x)] # Convert to readable
    # Find single nearest neighbor for each Y point
    idxs_y_x, dists_y_x = knn_search(hnsw_x, Y, 1)
    idxs_y_x = [convert.(Int, idxs_y_x[x]) for x in 1:length(idxs_y_x)] # Convert to readable
    ...
    Dxk_loc = zeros(length(Y), n)
    Dyk_loc = zeros(length(Y), n)
    for k in 1:length(Y)
