...
tri_cells = [MeshCell(VTKCellTypes.VTK_TRIANGLE, tri_elements[x]) for x = 1:length(tri_elements)]
...
for i = 1:marker_regions
    ...
    bc_normal, bc_tangent, bc_x_normal, bc_y_normal = calculatenormal(Y_points, bc_elements)
    ...
    # Generate BC cells
    bc_cell = [MeshCell(VTKCellTypes.VTK_LINE, bc_elements[x]) for x = 1:length(bc_elements)]
...
Y_idx_bc_g = [Y_idx_bc[x] .+ Y_idx_bc_max .- int_range[2] for x=1:length(Y_idx_bc)]
for i = 1:marker_regions
